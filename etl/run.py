...
def extract(_env):
    ...
    good_df_raw = pd.read_parquet("hf://datasets/OpenAssistant/oasst2/data")[['text', 'lang', 'role']]
...
def Transform(bad_df_raw, good_df_raw):
    ...
    g_df_raw = SparkSession.createDataFrame(good_df_raw).filter(F.col('role') == 'prompter').withColumns({
        "label" : F.lit(0)
    }).select('text', 'label')
